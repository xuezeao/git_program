...
void Sheet_OperatePage::specialWrite(int order,int error, int i) //0：未操作 1：错误 2：移动
{
    ...
    if(order == 0)
    {
        query.exec(QString("update %4 set judgeAttitude='%1' where drawerNo=%2,positionNo=%3").arg(err).arg(sheet_drawer[sheet_model]).arg(i).arg(T_tableexecute->T_tablesheet));
