...
void popupPage::showNeedPlaceReagent()
{
    ...
    if(rowAllNum >= acount)
    {
        ...
        model->select();
        QString showName = query.value(0).toString();//名字
        QString showVolume = query.value(2).toString();
        QString showLocation = query.value(1).toString();
...
void popupPage::on_pushButton_placedNext_clicked()
{
    ...
    model->setData(model->index(0,3),"已放");//更改状态
